...
The MCP server requires Python 3.11 or newer. If the `docgen-mcp` command is installed into `~/.local/bin`, make sure that directory is on `PATH` for Claude Code, or use the absolute command path in the MCP configuration.
